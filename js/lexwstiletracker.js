...
function loadjQueryui() {
	...
	if (typeof jQuery.ui == 'undefined') {
		...
		loadR3Script(
				"http://www.dalevross.com/scrabtourneyasst/js/jquery-ui-1.8.20.custom.min.js",
				function() {
					if(getTilesLeft())
					{
...
//				    },									
	...
	var loadinghtml = '<div><span>Loading...</span><br/><img src="http://www.dalevross.com/scrabtourneyasst/trackerloading.gif" /></div>';
	if ($dialog.dialog('isOpen')) {
		$dialog.html(loadinghtml);
	} else {
		$dialog.html(loadinghtml).dialog('open');
	}
	$.ajax({url : 'http://www.dalevross.com/scrabtourneyasst/scrabtiletracker.php?callback=?',
				context : this,
				data : (params),
				dataType : "jsonp",
				success : function(data) {					
					...
					var background = 'http://www.dalevross.com/scrabtourneyasst/' + game + 'board-smt.png';
...
function runTileTracker()
{
	...
	if (typeof jQuery == 'undefined') {
		loadR3Script("http://code.jquery.com/jquery-1.7.min.js",
				function() {
	
					$('head').append('<link rel="stylesheet" href="http://www.dalevross.com/scrabtourneyasst/css/smoothness/jquery-ui-1.8.20.custom.css" type="text/css" />');
					loadjQueryui();
				});
	} else {
		loadjQueryui();
